...
def criar_suprimento(
    ...
    db.commit()
    db.refresh(novo)
    registrar_log(db, usuario_atual["email"], "Suprimento", novo.id, "CREATE", f"Criado suprimento {novo.nome}")
    return novo
...
def listar_suprimentos(db: Session = Depends(get_db), usuario_atual=Depends(obter_usuario_atual)):
    ...
    registrar_log(db, usuario_atual["email"], "Suprimento", acao="READ", detalhes="Listagem de suprimentos")
    return lista


@roteador.put("/suprimentos/{id}", response_model=SuprimentoResponse)
def atualizar_suprimento(id: int, dados: SuprimentoBase, db: Session = Depends(get_db),
                         usuario_atual=Depends(obter_usuario_atual)):
    if usuario_atual["papel"] != "ADMIN":
        raise HTTPException(status_code=403, detail="Acesso negado")
    ...
    db.commit()
    db.refresh(sup)
    registrar_log(db, usuario_atual["email"], "Suprimento", id, "UPDATE", f"Atualizado suprimento {id}")
    return sup


@roteador.delete("/suprimentos/{id}")
def remover_suprimento(id: int, db: Session = Depends(get_db), usuario_atual=Depends(obter_usuario_atual)):
    if usuario_atual["papel"] != "ADMIN":
        raise HTTPException(status_code=403, detail="Acesso negado")
    sup = db.query(m.Suprimento).filter(m.Suprimento.id == id).first()
    if not sup:
    ...
    db.delete(sup)
    db.commit()
    registrar_log(db, usuario_atual["email"], "Suprimento", id, "DELETE", f"Removido suprimento {id}")
